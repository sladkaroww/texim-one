...
export async function onRequest(context) {
    ...
    if (!env.TEXIM_INVITES) return json({ success: false, message: 'Invite storage is not configured.' }, 500);
    const token = new URL(request.url).searchParams.get('token') || '';
    if (!/^[a-f0-9]{64}$/i.test(token)) return json({ success: false, message: 'Invalid invitation token.' }, 403);
    const invite = await env.TEXIM_INVITES.get(`invite-${token}`, { type: 'json' });
