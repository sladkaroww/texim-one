...
function normalize(item) {
    const dateRaw = item.created_at || item.createdAt || item.date || item.published_at || '';
    return {
        id: item.id,
        title: item.title || item.name || 'TEXIM ONE News',
        date: String(dateRaw).slice(0, 10),
        summary: item.content_summary || item.summary || item.excerpt || '',
        image: item.image || item.banner || item.cover || '',
        url: `https://truckersmp.com/vtc/${VTC_ID}/news/${item.id}`,
    };
}

export async function onRequest() {
    try {
        const res = await fetch(`https://api.truckersmp.com/v2/vtc/${VTC_ID}/news`, {
            headers: { 'User-Agent': 'TEXIM-ONE-Site/1.0' },
        });
        if (!res.ok) throw new Error('TruckersMP unavailable');

        const json = await res.json();
        const raw = Array.isArray(json.news) ? json.news : (Array.isArray(json.response) ? json.response : []);
