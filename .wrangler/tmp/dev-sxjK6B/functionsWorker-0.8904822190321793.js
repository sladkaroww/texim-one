...
// .wrangler/tmp/pages-WRmjvI/functionsWorker-0.8904822190321793.mjs
var __defProp2 = Object.defineProperty;
var __name2 = /* @__PURE__ */ __name((target, value) => __defProp2(target, "name", { value, configurable: true }), "__name");
var VTC_ID = "74050";
var USER_AGENT = "TEXIM-ONE-Site/1.0 (official website; via Cloudflare Pages)";
...
__name(tmFetch, "tmFetch");
__name2(tmFetch, "tmFetch");
function toDate(str) {
...
async function onRequest() {
  try {
    ...
    [...organized, ...attending].forEach((e) => {
      ...
      if (start.getTime() < now.getTime() - 60 * 60 * 1e3) return;
      seen.add(e.id);
      const duration = e.duration || 3 * 60 * 60 * 1e3;
      const end = new Date(start.getTime() + duration);
      const location = e.departure && e.departure.location ? [e.departure.location, e.departure.city].filter(Boolean).join(", ") : e.server ? e.server.name : "";
...
var VTC_ID2 = "74050";
var USER_AGENT2 = "TEXIM-ONE-Site/1.0 (official website; via Cloudflare Pages)";
var TIMEOUT_MS = 8e3;
var FALLBACK_EVENTS = [
  {
    id: 33929,
...
async function tmFetch2(path) {
  const url = `https://api.truckersmp.com/v2/vtc/${VTC_ID2}${path}`;
...
__name(tmFetch2, "tmFetch2");
__name2(tmFetch2, "tmFetch");
function toDate2(dateStr) {
  return /* @__PURE__ */ new Date(dateStr.replace(" ", "T") + "Z");
}
__name(toDate2, "toDate2");
__name2(toDate2, "toDate");
function normalize(item) {
  const start = toDate2(item.start_at);
  const now = /* @__PURE__ */ new Date();
  if (start.getTime() < now.getTime() - 60 * 60 * 1e3) return null;
  ...
  return {
    ...
    meetupAt: item.meetup_at ? toDate2(item.meetup_at).toISOString() : start.toISOString(),
    departure: item.departure ? { location: item.departure.location, city: item.departure.city } : null,
    arrive: item.arrive ? { location: item.arrive.location, city: item.arrive.city } : null,
...
var FALLBACK_WEBHOOK = "https://discord.com/api/webhooks/1537097067351122020/RT9l-s5klLMRH6f30Hp7ZwiX9L7TjEqgKnbLa3OYQooLfgbd2_ZkyY8zGTQAq10_pz8I";
...
// .wrangler/tmp/bundle-Vk8hRZ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Vk8hRZ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8904822190321793.js.map
